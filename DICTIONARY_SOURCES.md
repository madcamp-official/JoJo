...
- 한 sense 안에 품사가 2개 동시에 붙는 경우 있음 — `pos`는 단일값이라 더 대표적인 쪽 하나만, 원본 조합은 `posRaw`에 보존.
...
**상태**: `[2026-07-28 재실측 시도 — 접근 차단으로 실패]` 웹 검색으로 실제 존재하는 페이지 URL을 여러 개 확보(`zdic.net/hans/至行`, `zdic.net/hans/行处`, `zdic.net/hans/花` 등 검색 결과에 색인돼 있음)했지만, **자동화 요청(fetch 도구)으로는 이 URL들이 전부 HTTP 404를 반환** — 반면 사이트 루트(`zdic.net/`)는 정상 응답함. 즉 汉典은 루트는 열려 있어도 개별 단어 페이지는 브라우저가 아닌 요청을 걸러내는 것으로 보임(정확한 차단 방식은 불명 — User-Agent 검사, 봇 감지, 또는 JS 렌더링 필요 등 가능). **실제 스크래퍼 구현 시 이 차단을 감안한 대비(User-Agent 지정, 필요시 헤드리스 브라우저 등)가 필요하다는 것 자체가 이번 재실측의 실질적 소득**이다. 응답 구조 자체는 여전히 미확인 — 유의어(近反义词 섹션) 존재는 초기 조사 수준으로만 알려져 있고 실제 필드/HTML 구조는 확인 못 함.
...
- `heteronyms`(복수) — 동자이음. **"行" 실측 결과 heteronyms는 3개(háng/xíng/xìng)** `[정정 — 기존 기록은 "4개, háng/hàng/xíng/xìng"였는데 hàng 발음 항목은 실제 응답에 없음]`. 각 heteronym이 다른 병음에 다른 뜻풀이 세트를 가짐 → `DictionaryReading[]`으로 매핑하는 방침은 유지.
- `link` 필드(관련어 참조) — **"蟑螂" 실측: `definitions[0].link: ["也稱為「蜚蠊」。"]`로 정확히 재확인됨.** 단, 기존 기록엔 없던 사실이 하나 확인됨: **`link`는 문자열 하나가 아니라 문자열 배열**(`string[]`) `[세부 정정]` — 값 자체("也稱為「蜚蠊」")는 정확히 일치. `DictionarySense.seeAlso`로 매핑하는 방침은 유지, 다만 어댑터가 `link[]` 배열 전체를 순회해야 함.
...
- **汉典 원본 응답의 전체 구조** — 2026-07-28 재실측 시도했으나 자동화 요청이 차단(404)돼 실패, 여전히 미확인.
...
- zh 이합사(离合词, 结婚/见面처럼 중간에 성분 삽입 가능한 특이 문법) — CC-CEDICT는 태깅 없음으로 확인됨(재확인), 汉典/萌典은 접근 제약으로 여전히 미확인.
