...
**⚠️ 재실측 정정(2026-07-28, 汉典 어댑터 실제 구현 시점 — curl로 打/打算/结婚/一石二鸟 4개 페이지 재확인 및 `/hant/一石二鳥`로 zh-Hant 경로도 확인).** 아래 내용이 위 이전 기록(같은 날짜지만 더 이른 시점 재실측)을 대체한다 — 이전 기록이 서술한 "`gy-reading`/`gy-sense-list`가 페이지 전체의 유일한 뜻풀이 구조"라는 전제 자체가 지금 라이브 페이지와 다르다(사이트 템플릿이 그 사이 바뀌었거나, 최초 재실측이 어떤 이유로 실제와 다른 렌더를 관측한 것으로 보임 — 원인은 확인 불가, 지금부터는 이번 재실측을 최신 정본으로 취급한다):

- 표제어 페이지는 탭 4개(`#jbjs` 基本解释, `#xxjs` 词语解释, `#gyjs` 国语辞典, `#syn` 近反义词 — `#trans` 翻译도 있으나 스키마 밖)로 나뉘고, **어댑터는 `#gyjs`(国语辞典, 대만 교육부 계열 데이터 — 萌典과 같은 데이터 축) 섹션 하나만 채택**한다. 이유: `#jbjs`는 한자 단독 표제어(`打`)에만, `#xxjs`는 단어/성어 표제어(`打算`)에만 있어 표제어 종류별로 별도 파서가 필요해지는데, `#gyjs`는 한자·단어·성어 표제어 전부(打/打算/结婚/一石二鸟 4개 페이지 전수 확인)에 공통으로 존재하고 sense 단위 품사·근의어·반의어까지 구조화돼 있어 셋 중 파싱 품질이 가장 높다. `#syn` 섹션은 `#gyjs` 안의 sense별 근반의어를 표제어 전체로 합친 롤업이라(값 대조로 확인) 중복 — 스킵.
- `#gyjs` 안 구조: `.gy-reading`(발음 그룹, `.gy-reading__py`에 병음) 여러 개 → 각 발음 그룹 안에 `<section class="gy-pos">`(품사 배지 `.gy-pos__badge` + `.gy-sense-list`)가 0개 이상 있고, 배지가 없으면 `.gy-sense-list`가 바로 옴. `.gy-sense-list` 안 `.gy-sense`(뜻풀이 하나, `.gy-sense__def`)가 형제 노드로 나열.
- **다음자(多音字) 구조 재확인**: `打`는 dǎ/dá 2개 발음, 각 발음 아래 품사 배지가 여러 개 붙을 수 있음(dǎ 밑에 动/介 2개, dá 밑에 名 2개 — 총 4개 `gy-pos` 그룹, 22+1=23개 뜻풀이) — `DictionaryReading[]`에 발음 단위로 대응.
- **품사(词性) — 이전 기록("필드 자체가 없음")을 반증.** `打`(한자, 다음자) 표제어는 `gy-pos__badge`로 품사가 붙는다(动=동사, 介=전치사, 名=명사 등 확인). 단, `打算`/`结婚`/`一石二鸟`(단어/성어) 표제어는 이 배지가 아예 없어 `pos`가 undefined — 즉 "품사 유무"가 소스 전체가 아니라 **표제어가 다음자 한자인지 여부**에 달려 있다.
- 근의어/반의어는 `#gyjs`의 sense 본문 안에 `<div class="xxjs-also">`(label `xxjs-block-label--syn`/`--ant`) 블록으로 sense 단위로 직접 붙어 있어(`.syn-tag` 링크), 위에서 스킵한 `#syn` 롤업 섹션 없이도 sense 레벨 synonyms/antonyms를 그대로 뽑을 수 있다(재확인: `打算` 1번 뜻에만 근의어 7개, 2번 뜻엔 없음 — sense별로 갈림. `结婚`은 근의어 6개+반의어 3개. `一石二鸟`는 근의어 2개).
- 예문은 두 가지 다른 마크업으로 온다: 단어/성어(`打算`/`一石二鸟`)는 `xxjs-also`(label `--also`, 텍스트 `.xxjs-also__text`, "如：「...」" 형태), 한자(`打`)는 `gy-sense__eg`(텍스트 `.gy-sense__eg-text`) — 어댑터는 두 패턴 다 처리.
- 고전 인용(书证, `gy-sense__cit`/`gy-sense__cit-list`, 저자·출전 포함 — 예: "究竟上天不生无禄的人，等慢慢再打算就是了。 ——《官话指南·卷一·应对须知》")은 여전히 실측 확인되나(이번엔 `#gyjs`가 아니라 스킵한 `#xxjs`/`#jbjs` 쪽에서 발견, `#gyjs`엔 이 인용 블록이 없음) 어차피 채택 안 한 섹션이라 이번 구현 스코프에선 자연히 배제됨 — 출전 메타데이터 스키마 미반영 결정 자체는 유지(아래 문단).
- **다단계(계층형) 뜻 구조 없음 — 재확인.** `#gyjs`의 `.gy-sense` 목록은 항상 평평한 형제 구조(打 dǎ: 19+3=22개 전부 동일 레벨, 打算: 2개, 结婚/一石二鸟: 1개, `gy-sense__num` 없는 단일 뜻 케이스 포함) — `DictionarySense.parentIndex`는 이 소스에서 항상 undefined로 유지.
- `CL:`(양사) 마커나 CC-CEDICT식 명시적 태그는 `#gyjs`에선 안 보임(명사류 단어 CL: 표기는 CC-CEDICT 쪽 축).
- **이번 구현에서 미반영(스코프 밖)**: 고전 인용 출전 메타데이터(위 문단, Kotobank 精選版 `[初出の実例]`와 동일 축의 문제라 스코프 결정을 미룸 — en/ja/zh 전체에서 "평문 예문"과 "출전 붙은 고전 인용"을 같은 필드에 넣을지부터 정해야 함), 상용 词组(복합어 목록) · 다국어 翻译 섹션 · 간체/번체 대응형 병기 · 부수/획수/자형/설문해자/음운방언(학습자용 팝업사전과 무관 판단) · "同音词"(클라이언트 JS 전용, 정적 HTML엔 없음) — 전부 대응 필드가 스키마에 없거나 이 앱 기능과 무관하다고 판단해 그대로 둠.

**스키마 매핑**: `#gyjs` 섹션만 파싱 — `.gy-sense__def` → `gloss[]`, `.gy-reading__py` → `pronunciations[].value`, `.gy-pos__badge` → `pos`(다음자 한자 표제어만, 그 외 undefined) + `posRaw`(배지 원문), sense 안 `.xxjs-also`(label `--syn`/`--ant`)의 `.syn-tag` → `synonyms`/`antonyms`, `.xxjs-also__text`(label `--also`)/`.gy-sense__eg-text` → `examples`. `isIdiom`은 `#gyjs` 안에 성어 전용 마커가 없어 이 소스는 항상 undefined(CC-CEDICT `(idiom)` 라벨과 달리 汉典은 이 축을 안 씀).
...
- **en.wiktionary.org REST API(definition 엔드포인트)가 일부 표제어에서 `zh` 언어 블록 자체를 안 주는 원인** — 실측(2026-07-28): `水`/`一` 같은 단일 한자는 wikitext 상 `==Chinese==` 섹션과 `{{zh-pron}}`이 분명히 존재하는데도 REST API 응답엔 `zh` 키가 아예 없다(`捷運`/`打算`/`你好` 같은 복합어는 정상). 단일 한자 표제어 특유의 페이지 구조(다의어·이독 섹션이 유난히 많고 복잡함)가 REST API 파서를 실패시키는 것으로 추정되나 확정은 못 함 — Wiktionary 어댑터(en/ja/zh 최종 폴백)가 zh 단일 한자 표제어에서 구조적으로 약하다는 뜻이라 정식 폴백 오케스트레이션 구현 시 참고 필요(汉典/CC-CEDICT가 앞순위라 실사용 영향은 제한적).
