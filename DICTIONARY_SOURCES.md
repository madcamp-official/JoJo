...
- **다단계(계층형) 뜻 구조 없음 — 실측 확인(2026-07-28, 打/打算/结婚/一石二鸟 4개 페이지 curl 재실측).** `打`(dǎ 발음, 19개 뜻)은 `gy-sense__num` 1~19번이 전부 같은 레벨의 `div.gy-sense` 형제 노드로 나열되고, `[一]`/`㈠` 같은 상위 그룹 마커나 sense 안에 중첩된 하위 sense가 코드상 전혀 없음(직접 파싱해 중첩 0건 확인). `打算`도 2개 뜻풀이가 평평한 형제 구조. `结婚`/`一石二鸟`는 `gy-sense__num` 마커 자체가 없는 단일 뜻 케이스. 즉 汉典은 `DictionarySense.parentIndex`가 항상 undefined인 소스로 확정 — 아래 "공통 스키마 결정 사항"의 `parentIndex` 신설 노트에 있던 "汉典도 결국 마주칠 문제로 예상"이라는 추정은 이 실측으로 반증됨(정정 반영).
