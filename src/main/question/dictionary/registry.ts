...
// 담당 공동 — 사전 소스 직접 선택 드롭다운(정식 기능, 2026-07-30 격상)의 자동 감지 기반.
// en/ja/zh 어댑터가 각자
// `question/dictionary/{merriamWebster,oewn,daijisen,jmdict,hanyu,guoyuCidian,cccedict,wiktionary}.ts`
// 파일로 구현돼 있다 — 이 파일은 "그 파일이 실제로 존재하는가"를 Vite 의
// `import.meta.glob`(빌드 타임에 디렉터리를 스캔, 없는 파일을 나열해도 에러 안 남)으로
// 감지해 재등록 없이도 자동으로 목록을 채운다. 실제 폴백 오케스트레이션은
// `question/dictionary.ts`의 `FALLBACK_CHAINS`가 담당하고, 이 파일은 "무엇이 구현돼
// 있는지 알아내서 드롭다운에 보여주는" 역할만 하며, 팝업의 "직접 선택" 토글을 켰을
// 때만 여기서 감지한 소스를 조회하는 데 쓰인다(꺼져 있으면 드롭다운 선택과 무관하게
// 정식 폴백 체인이 실행됨).
//
