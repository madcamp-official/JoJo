...
/**
 * 세로쓰기 열 하나를 manga-ocr 대신 PaddleOCR 로 인식한다 — 실측 비교(합성 세로쓰기
 * 8줄): 같은 조건(워커 풀 3개, 병렬)에서 PaddleOCR 1.6초 vs manga-ocr 7.6초로 약 4.6배
 * 빨랐고, 회전 트릭 없이도(세로쓰기는 글자 자체는 똑바로 서 있고 읽는 방향만 위→아래라
 * PaddleOCR 인식 모델이 그대로 정확히 읽음) 정확도도 100% 일치했다. manga-ocr 은 망가
 * 말풍선 같은 손글씨풍 폰트 전용으로 남겨두고(ocrManga.ts, 지금은 기본 경로에서 안 씀 —
 * 필요해지면 다시 연결), 일반 활자 세로쓰기(소설/책 PDF 등)는 이 함수를 기본으로 쓴다.
 *
 * detectLinesWithPaddle 로 줄 위치를 먼저 찾고(위→아래 정렬 — 세로쓰기 열의 읽기 순서),
 * 줄마다 recognizeWithPaddle 을 병렬로 호출한다 — recognizeWithPaddle 을 열 전체에 한
 * 번만 호출해도 PaddleOCR 자체 검출기가 내부적으로 텍스트를 찾긴 하지만, 그 결과 순서가
 * 세로쓰기 읽기 순서를 보장한다는 근거가 없어서(가로쓰기 기준으로 설계된 정렬) 줄 단위로
 * 직접 나눠 순서를 우리가 확정짓는 쪽을 택했다.
 */
export async function recognizeVerticalColumnWithPaddle(
  ...
  const perLine = await Promise.all(lines.map((line) => recognizeWithPaddle(image, language, line)))
  if (perLine.some((words) => !words)) return null // 줄 하나라도 실패하면 통째로 null(호출부가 Tesseract 로 폴백)
