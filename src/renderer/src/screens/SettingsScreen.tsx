  ...
  return (
    <div className="screen settings-screen">
      ...
      <section className="settings-section">
        <h2>언어 선택</h2>
        <p className="desc">
          자동: 자막/화면 텍스트로 언어를 매번 자동 판별합니다. 자막·웹 텍스트처럼 실제
          문자를 바로 읽어 판별하는 경로는 정확도가 높지만, 화면 캡처(OCR) 경로는
          Tesseract가 문자를 인식하기 전에 문자 체계(스크립트)부터 추정해야 해서 폰트나
          레이아웃에 따라 이 추정 자체가 틀리는 경우가 있어 상대적으로 정확도가 낮습니다.
          라틴·키릴·아랍·데바나가리 문자를 공유하는 언어(예: 프랑스어/독일어/스페인어 등)가
          섞인 콘텐츠에서는 특히 판별이 흔들릴 수 있습니다. 특정 언어를 선택하면 이 판별
          자체를 건너뛰고 항상 해당 언어로 확정합니다 — 자동 판별이 계속 틀리는 콘텐츠를
          볼 때 유용합니다.
        </p>
        <p className="desc">
