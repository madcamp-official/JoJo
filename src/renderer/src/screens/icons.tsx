...
import { Pencil, Trash2, Settings, FolderOpen, BookOpen, HelpCircle, ArrowLeft, Sun, Moon, ScrollText, Copy } from 'lucide-react'
...
// 연필/휴지통/설정 톱니는 lucide-react(Pencil/Trash2/Settings)로 통일.
...
export function PencilIcon({ size = 16 }: { size?: number }) {
  return <Pencil size={size} strokeWidth={2} aria-hidden="true" />
}

export function TrashIcon({ size = 16 }: { size?: number }) {
  return <Trash2 size={size} strokeWidth={2} aria-hidden="true" />
}
...
export function BookIcon({ size = 20 }: { size?: number }) {
  return <BookOpen size={size} strokeWidth={2} aria-hidden="true" />
}
