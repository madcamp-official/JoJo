...
외국어 원서·영상·웹소설을 볼 때 모르는 표현을 만나면 뷰어를 벗어나 사전·번역기로 이동해야 하고, 사전은 문맥 속 의미까지는 알려주지 못한다. Nuance는 사용자가 보고 있는 창 위에 투명 오버레이를 띄워, 콘텐츠를 벗어나지 않고 단어를 클릭하면 문맥을 이해한 AI 답변을 받을 수 있게 한다. 텍스트를 직접 추출할 수 있으면 추출하고, 불가능하면(스캔본·이미지 전자책) OCR로 대응한다.

## 다운로드 및 실행

[Releases](../../releases)에서 최신 버전을 받아 실행한다. `v*.*.*` 태그가 푸시되면 GitHub Actions가 Windows/macOS 설치 파일과 크롬 확장 zip을 빌드해 같은 릴리스에 올린다(초안으로 생성되며, 릴리스 노트 작성 후 관리자가 최종 공개한다).
...
유튜브/넷플릭스 자막이나 일반 웹페이지 DOM 추출을 쓰려면 확장도 함께 등록한다.

1. `nuance-extension-*.zip`을 받아 압축을 해제한다.
2. Chrome(또는 Chromium 계열 브라우저)에서 `chrome://extensions`로 이동해 우상단 **개발자 모드**를 켠다.
3. **압축해제된 확장 프로그램을 로드합니다**를 눌러 압축 해제한 폴더를 선택한다.
4. 데스크톱 앱이 켜져 있어야 확장이 로컬 WebSocket으로 앱과 연결된다 — 앱을 먼저 실행한 뒤 브라우저를 사용한다.

### 첫 실행 설정

- AI 발음/사전/통합 질문 기능을 쓰려면 앱 실행 후 설정 화면에서 GPT/Gemini/Claude 중 하나의 API 키를 등록해야 한다(키는 `safeStorage`로 로컬에만 암호화 저장되고 외부로 전송되지 않는다).
- 레이아웃 분류·PaddleOCR·일본어 특화 OCR(Yomitoku/NDLOCR-Lite) 엔진은 설치 파일에 포함돼 있지 않고, 해당 기능을 처음 쓰는 시점에 인터넷에서 자동으로 내려받아 캐싱한다(수백MB~2GB대, 최초 1회, 이유는 [python/README.md](python/README.md) 참고). 다운로드 중에도 기본 Tesseract 경로로 계속 동작하며, 실패해도 앱이 죽지 않고 해당 기능만 품질이 떨어진 채로 동작한다.
...
- **직접 추출 우선, OCR은 폴백** — 유튜브·넷플릭스 자막, 웹페이지 DOM, Nuance 자체 뷰어로 연 txt/epub/PDF, macOS 프리뷰 PDF(접근성 API)는 원문을 그대로 읽고, 그 외(스캔본, Kindle/Apple Books 등 외부 뷰어)는 OCR로 처리한다.
...
| 채널 | 대상 | 텍스트 확보 방식 |
|---|---|---|
| 데스크톱 | txt/epub/PDF(Nuance 자체 뷰어로 연 경우) | Nuance 자체 뷰어로 원본 파일 직접 추출 |
| 데스크톱 | PDF(macOS 프리뷰, 텍스트 레이어 있음, 창 선택으로 연 경우) | 접근성 API(AX)로 텍스트+좌표 직접 획득 |
| 데스크톱 | 그 외 창 선택 대상(Kindle/Apple Books 등 외부 뷰어, 스캔 소설 등) | OCR (전자책 뷰어 접근성 API 연동은 아직 미구현) |
| 브라우저 | 유튜브 | URL 기반 원어 자막 추출 |
| 브라우저 | 넷플릭스 | 확장 프로그램으로 현재 에피소드 원어 자막 추출 |
| 브라우저 | 웹소설·웹툰·일반 웹페이지 | 확장으로 본문 DOM 텍스트 직접 추출 → 부족하면 OCR |
...
유튜브/넷플릭스 자막, 일반 웹페이지 DOM 추출, Nuance 자체 뷰어(txt/epub/PDF) 직접 추출, macOS 프리뷰 PDF 접근성 API(AX) 추출, LLM 3종 어댑터·스트리밍, 발음/사전(다중 소스 폴백)·통합 질문, 설정 화면, 언어 자동 감지(tier1+tier2), GitHub Actions 릴리스·자동 업데이트는 구현이 완료된 상태다. Kindle/Apple Books 등 외부 전자책 뷰어의 접근성 API 연동과, 창 선택으로 연 임의의 PDF 뷰어에서 텍스트 양 기반으로 직접 추출/OCR을 자동 판정하는 기능은 아직 미구현이다(현재는 OCR로 처리).
