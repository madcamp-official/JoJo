...
| `Nuance-*.dmg` / `Nuance-*.zip` | macOS 앱 |
| `nuance-extension-*.zip` | 브라우저 확장(압축해제 후 "압축해제된 확장 프로그램 로드"로 등록) |

- 정식 코드 서명 없는 사설 배포라 첫 실행 시 경고가 뜰 수 있다 — Windows는 SmartScreen에서 "추가 정보 → 실행", macOS는 앱을 우클릭 후 "열기"로 실행한다.
- 설치 후 업데이트: Windows는 `electron-updater`로 새 버전을 자동 다운로드·설치한다. macOS는 서명이 없어 자동 설치 대신 새 버전이 나오면 알림만 띄우고, 클릭하면 릴리스 페이지로 안내한다.
- AI 발음/사전/통합 질문 기능을 쓰려면 첫 실행 후 설정 화면에서 GPT/Gemini/Claude 중 하나의 API 키를 등록해야 한다.
