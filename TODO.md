...
**앱 · 윈도우 · 모드**
- [x] ~~창 선택 UI + `desktopCapturer` 창 목록/선택~~ — **macOS는 `desktopCapturer` 대신 자체 열거로 교체**(`capture.ts: listWindowsMac`). `desktopCapturer`는 내부적으로 `kCGWindowListOptionOnScreenOnly`라 다른 가상 데스크탑(Space)의 창이 애초에 목록에서 빠지는 문제가 있어, `macWindow.ts: listAllMacWindows`가 `CGWindowListCopyWindowInfo`를 `kCGWindowListExcludeDesktopElements`(OnScreenOnly 미포함)로 호출해 Space 와 무관하게 전체 창을 열거하도록 변경(실기기에서 onScreenOnly 46개 vs 전체 222개로 차이 확인, 다른 Space의 실제 창을 `screencapture -l<id>`로 화면 전환 없이 캡처되는 것도 검증함). 각 창 썸네일은 여전히 `screencapture -l<id>`(`captureMacWindowById`)로 얻는데, 다른 Space 창은 캡처가 느리거나 실패할 수 있어 4초 타임아웃 + 실패 시 자리표시자 썸네일로 대체(선택 자체는 계속 가능). Windows는 기존 `EnumWindows`(win32Capture) 그대로 — 가상 데스크탑 지원 여부는 별도 확인 필요(TODO).
...
- [x] 투명·클릭스루 오버레이 윈도우 (선택 창에 정렬 · 이동/리사이즈 추적) — Windows: WinEventHook + 폴백 폴링. macOS: CGWindowList bounds 폴링(150ms)으로 정렬+이동/리사이즈 추적(win32 는 즉시 반응 훅까지 있고 mac 은 폴링만이라 반응이 약간 느릴 수 있음). z-order 재적용(`syncOverlayZOrder`)은 원래 포그라운드 전환 이벤트에서만 돌았는데, 메모장 탭 전환처럼 같은 창 안에서 내부적으로 다시 그려지는 경우(포그라운드 이벤트가 안 뜸) 오버레이가 뒤로 밀리는 문제가 있어 150ms 폴백 폴링에도 같이 넣음(`windows.ts: trackSelectionOverlay`).
