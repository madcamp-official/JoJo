...
**선택 · 좌표 매핑**
- [ ] 단어 bbox 좌표 확보 → 커서 좌표 ↔ 단어 매핑 — 매핑 로직(`shared/wordMapping.ts: findWordAtPoint`)과 오버레이 연동은 완료. 단, 실제 추출 파이프라인(OCR/직접 추출)이 아직 없어서 `Overlay.tsx`의 `MOCK_WORDS`(하드코딩된 자리표시자 6개)로 동작 검증만 한 상태 — "bbox 좌표 확보" 자체(실제 텍스트 인식)는 여전히 미구현. 추출 파이프라인이 준비되면 `MOCK_WORDS`를 IPC로 받은 실제 `Word[]`로 교체 필요.
