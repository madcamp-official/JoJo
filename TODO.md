...
**앱 · 윈도우 · 모드**
- [x] ~~창 선택 UI + `desktopCapturer` 창 목록/선택~~ — **macOS는 다른 가상 데스크탑(Space)의 창도 선택 가능하도록 확장**(`capture.ts: listWindowsMac`). `desktopCapturer`는 내부적으로 `kCGWindowListOptionOnScreenOnly`라 다른 Space의 창이 애초에 목록에서 빠지는 문제가 있어, 현재 Space는 그대로 `desktopCapturer`로 빠르게 일괄 조회(썸네일 포함)하고 거기 없는 창만 `macWindow.ts: listAllMacWindows`(`CGWindowListCopyWindowInfo`를 `kCGWindowListExcludeDesktopElements`로 호출, Space 무관 전체 열거)로 추가한다(실기기에서 onScreenOnly 46개 vs 전체 222개 차이 확인). 처음엔 전체를 `listAllMacWindows`로만 열거하고 창마다 `screencapture -l<id>`로 순차 캡처했는데, 창이 많으면(실기 130개+) 스폰 비용 때문에 목록이 뜨기까지 너무 오래 걸려("창 목록을 불러오는 중…"에서 멈춘 것처럼 보임) 지금 방식으로 변경 — 다른 Space 창은 목록 단계에서는 자리표시자 썸네일만 쓰고(선택 시엔 `screencapture -l<id>`로 원본 화질 캡처, 화면 전환 없이 되는 것도 검증함), 목록 자체는 `desktopCapturer` 배치 1회 + `listAllMacWindows` 1회로 빠르게 뜨도록 함. Windows는 기존 `EnumWindows`(win32Capture) 그대로 — 가상 데스크탑 지원 여부는 별도 확인 필요(TODO).
- [x] 선택된 창 테두리 색 표시 (일반=파랑 / 선택=보라) — Windows: 대상 창에 정렬+실시간 추적. **macOS: 구현됨** — `showMacSelectionOverlay`(`windows.ts`)가 CoreGraphics `CGWindowListCopyWindowInfo`(koffi, `selection/macWindow.ts`)로 창(`listAllMacWindows`가 열거한 windowId)의 실제 bounds 를 얻어 테두리를 정렬하고, `MAC_TRACK_INTERVAL_MS`(16ms) 고빈도 폴링으로 이동/리사이즈를 따라간다(Windows 폴백 폴링의 150ms보다 촘촘함). bounds 를 못 구하면(예: 창이 닫힘) 테두리를 숨긴다(`hideMacOverlay`) — 별도 디스플레이 전체 테두리 폴백은 없음. `MAC_OCCLUSION_EVERY`(6틱=~96ms)마다 z-순서를 확인해 대상 창이 다른 앱 창에 가리면 테두리를 숨긴다(`isMacTargetCovered`). **추가 권한 프롬프트 없음**(창 geometry 조회는 권한 불필요, 창 목록엔 이미 화면기록 권한 사용). osascript(자동화 권한) 방식은 unsigned dev 앱에서 프롬프트가 안 떠 폐기.
...
- [x] OCR 엔진 연동 — **범용 엔진(전체 언어 공통/자동감지용) + 언어별 최적 엔진(개별 특화) 이중 구조**로 결정.
  ...
  - [x] **창 캡처 크로스플랫폼** — OCR 엔진(Tesseract)은 PNG 버퍼만 받으므로 플랫폼 무관, 캡처만 분기. Windows: win32 `PrintWindow`. **macOS: 내장 `screencapture -o -l<windowID>`**(`capture.ts`, 네이티브 해상도라 PrintWindow 동급 품질). mac 은 물리 픽셀(Retina 2x)이라 `alignWordsToOverlay`(`extractionCache.ts`) darwin 분기에서 bbox 를 `scaleFactor` 로 나눠 오버레이 DIP 에 정합. `screencapture -l` 실기기 동작 확인(760×460 창 → 1520×920 PNG).
    ...
    - [ ] OCR 대상 영역 지정(바로 위 항목)은 아직 **Windows 전용** — 좌표 변환이 win32 API(`getCaptureOriginOffset` 등) 기반이라, mac 캡처가 붙었어도 영역 드래그 지정 자체는 mac에서 미구현 상태(전체 창 대상으로만 동작). 영역 내용 변화 감지(`changeWatcher.ts`)는 영역 지정에 의존하므로 동반 미구현.
...
- [ ] (담당 A) Windows — 다른 가상 데스크탑(Virtual Desktop)의 창 선택 지원 여부 미확인. `listWin32Windows`(`win32Capture.ts`)의 `EnumWindows` 자체는 가상 데스크탑과 무관하게 모든 최상위 창을 순회하지만, 열거 시 쓰는 `isCloaked()`(`DWMWA_CLOAKED`) 필터가 다른 가상 데스크탑에 있는 창까지 걸러내고 있는지 실기(Windows, 다중 가상 데스크탑 환경)로 확인 안 됨. 걸러진다면 `IsWindowOnCurrentVirtualDesktop`을 함께 확인해 걸러내는 대신 목록에 포함시키고, 캡처(`PrintWindow`/DWM Thumbnail)가 다른 가상 데스크탑 창에서도 실제로 되는지도 별도 검증 필요(COM `IVirtualDesktopManager` 바인딩 추가가 필요할 수 있음). macOS는 `desktopCapturer` → 자체 `CGWindowListCopyWindowInfo`(`kCGWindowListExcludeDesktopElements`) 열거로 교체해 다른 Space 창 선택 지원 완료(바로 위 항목 참고).
- [ ] (담당 A) 창 선택 목록 — macOS Space별 탭 구분 **보류**. "어느 Space에 창이 있는지"는 공개 API(`CGWindowListCopyWindowInfo` 등)로는 알 수 없고, 유일한 방법은 비공개(private) SkyLight 프레임워크의 `CGSCopySpacesForWindows`/`CGSMainConnectionID`(koffi로 로드는 됨) 뿐이다. 실기 테스트 결과 현재 활성 Space 의 창 1개만 정상적으로 space id 를 반환했고, 같이 조회한 나머지 20여 개 창(다른 Space·최소화된 창 등)은 전부 `null`을 반환해 소속 Space 를 알아낼 수 없었음 — macOS 버전에 따라 동작이 다르고 신뢰도가 낮은 비공개 API 라 이 위에 탭 UI 를 얹으면 "탭은 있는데 대부분의 창이 안 뜨거나 엉뚱한 탭에 뜨는" 상태가 될 위험이 큼. 사용자와 논의 후 보류 — 필요해지면 `CGSCopySpacesForWindows` 호출 인자(타입/마스크 조합)를 더 검증하거나 yabai 같은 오픈소스 도구의 구현을 참고해 신뢰도를 다시 확인할 것.
