  ...
  - [x] ~~사전 응답 통일 스키마 설계 및 타입 반영 — en/ja/zh 8개 소스(MW/WordNet/Wiktionary/Kotobank/JMdict/汉典/萌典/CC-CEDICT)가 전부 형식이 달라서(JMdict의 `v1`/`vt` 같은 EDICT 약어 코드, MW의 `sseq`/`dt`/`vis` 중첩 구조, CC-CEDICT의 슬래시 구분 평문 등), 각 어댑터가 원본을 파싱해 공통 타입으로 변환한 뒤에만 LLM에 넘기도록 `shared/types.ts`에 `CanonicalPos`/`DictionarySense`/`DictionaryEntry`/`DictionarySourceId` 추가(`llm/adapter.ts`가 GPT/Gemini/Claude를 `QuestionResult` 하나로 통일하는 것과 동일 패턴).~~
    ...
    - [ ] **사전 소스별 실측 노트** — 스키마·어댑터 설계 중 실제로 API/스크래핑해서 확인한 것들을 소스별로 정리(날짜는 확인 시점). 위 118~120번 항목의 미반영분과 이후 확인 내용을 여기로 통합함:
      - **공통(여러 소스에 걸친 스키마 결정)**:
        ...
        - (2026-07-28) `DictionaryEntry.isCommon` 필드는 결국 없앰 — 원래 "여러 후보 entry 중 하나를 고르는" 용도로 설계했는데, 실제로는 동일 표기(예: "橋")의 서로 다른 reading(はし/きょう)이 **하나의 entry로 병합**되는 게 맞는 설계라(MW hom, 萌典 heteronyms와 동일 패턴 — `DictionaryReading[]`로 그룹핑), 우선순위 신호의 실제 역할은 "entry 선택"이 아니라 "그 entry 안 어느 reading/sense가 대표인지"로 축소됨 — 그 역할에 맞는 자리(아래 JMdict 항목의 `DictionaryReading.isCommon`)로 다시 분리해서 부활시킴. (당초 함께 부활시켰던 `DictionarySense.tagCount`(OEWN 전용)는 2026-07-28 실데이터 확인 결과 폐기 대상으로 정정됨 — 아래 OEWN 항목 참고.)
      - **Merriam-Webster (MW, en)**:
        ...
        - **`uns`/`lbs` 확정(2026-07-28, `.env` 실키로 직접 호출)**: sense 단위 화용론적 사용법 설명 필드는 `uns`가 맞다 — "ain't hay" 실측: `dt` 배열 안에 `["uns", [[["text","used to say that an amount (of money) is a lot "],["vis",[...]]]]]` 형태로 옴 → `DictionarySense.usageNote`. `lbs`는 전문분야 라벨이 아니라 **일반 표기 관례 라벨**("deco" 실측: entry 최상위에 `lbs: ["often attributive"]`) — `sls`와 성격이 같아 `DictionarySense.usageTags`로 합류시키되, entry 최상위 값이라 어댑터가 그 entry의 모든 sense에 복제해 넣어야 함(레벨이 다름). 전문분야(컴퓨터/의학 등) 라벨은 MW 스키마에 별도로 없어 `domain`은 MW 소스에서 항상 undefined.
      - **OEWN (Open English WordNet, en)** — 공식 GitHub 저장소(`globalwordnet/english-wordnet`)의 Releases 에셋(`english-wordnet-2025-json.zip`)을 직접 다운로드해 로컬 번들(2026-07-28 실측: `en-word.net/static/...` 정적 링크는 503으로 죽어있지만 같은 파일의 GitHub Releases 직접 URL은 HTTP 200 정상 확인 — 미러 저장소는 불필요, 상세는 [DICTIONARY_SOURCES.md](DICTIONARY_SOURCES.md#oewn-open-english-wordnet) 참고). 아래 구조 기록은 `run`/`kick the bucket` 예시로 **기존에 남아있던 기록**이며, zip 내부 필드 구조 자체는 이번 2026-07-28 세션에서 파일을 직접 열람해 재검증하지는 못했다(DICTIONARY_SOURCES.md와 동일):
        ...
        - ~~`tagcount`(SemCor 실사용 빈도수, 실측: run(v) "달리다" 뜻 tagcount=106)가 sense마다 다르게 확인됨~~ → **정정(2026-07-28, GitHub Releases `english-wordnet-2025-json.zip`을 실제로 받아 압축 해제 후 전수 검사)**: `tagcount` 필드는 이 JSON 릴리스에 **존재하지 않는다**(`grep -r "tagcount"` 0건, sense 객체의 실제 키 26종 전수 확인해도 빈도 필드 없음 — Princeton WordNet WNDB `index.sense`의 `tag_cnt` 개념으로 추정되나 이 JSON 릴리스로는 못 가져옴). **`DictionarySense.tagCount?: number` 필드는 폐기 대상**(현재 데이터 소스로는 채울 방법이 없음, 필요하면 WNDB 원본 파일을 별도로 받는 방안을 재검토). JMdict의 `is_common`(엔트리=reading 그룹 단위로만 값이 있고 그 밑 sense는 전부 공유, 실측: jisho.org "上手")은 이 문제와 무관하게 `DictionaryReading.isCommon?: boolean`(JMdict 전용)으로 유지.
