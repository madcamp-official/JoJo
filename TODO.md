...
- [ ] 사전: 언어별 사전 API + 단어 분해 + LLM 뜻 번호 매핑 — **스텁만 존재**(`question/dictionary.ts` 가 빈 문자열 반환). 소스 구성은 확정 완료(PLAN.md §5), 아래는 구현 세부 작업 목록(아직 착수 전).
  ...
  - [x] ~~en 어댑터 — **Merriam-Webster**(1순위): 키 등록, 무료 개인용 티어~~ → `feat/dictionary-en`에 구현 완료(아직 `dev` 미병합) — `question/dictionary/mw.ts`(raw 타입 파싱 + `isRelevantEntry` 필터링 + `uros`/`cxs`/`uns`/`lbs` 처리, 세부 근거는 [DICTIONARY_SOURCES.md](DICTIONARY_SOURCES.md)의 MW 노트), `question/dictionary/senseSelect.ts`(번호매김+LLM 판정/번역+서식화), `question/dictionary.ts`(파이프라인 오케스트레이션 + 다중 단어 폴백, 아래 항목 참고).
  ...
  - [x] ~~다중 단어 선택 처리(en)~~ → **`feat/dictionary-en-multiword`에 구현 완료, `feat/dictionary-en`에 병합됨(2026-07-28, 아직 `dev` 미병합)**. 최종 설계는 초기 계획(아래 취소선 4단계)과 달리 "atom 분해 결과 재사용"이 아니라 **단어 하나짜리 파이프라인(MW 조회→LLM 판정/번역→서식화)을 그대로 병렬 재사용**하는 쪽으로 갔다 — 단어별 뜻이 서로 독립적이라(관용구가 아닌 이상) 하나의 프롬프트/판정으로 합칠 필요가 없었기 때문:
