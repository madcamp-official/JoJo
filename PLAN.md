...
4. **AI 주변 범위(Byte)** — 프롬프트 제출 시 함께 넘길 앞뒤 텍스트 범위를 Byte 단위로 지정(자유 지정: 연속 슬라이더 + 숫자 입력, 상한 4096). 앞/뒤 예산을 분리해 각각 지정하거나 잠금(linked)으로 동일 값 사용. 실제로는 순수 바이트 경계에서 문장이 잘리지 않도록 문장 경계까지 확장된다. 미리보기에 "포함 제외 / 사용자 선택 영역 / 포함될 주변 범위"를 색으로 시각화.
5. **언어 선택** — 자동 언어 감지 / 직접 선택(영어·일본어·중국어). OCR 언어 설정.

**팝업 화면**: 상단에 선택된 원문 문맥 표시(선택 앞뒤 각 1024바이트, 문장 경계까지 확장·부족하면 있는 만큼만, 내부 스크롤 없이 텍스트 양만큼 높이 자동 확장) → 툴바(LLM 아이콘 · [발음]·[사전 검색] 체크박스 · [입력](통합 질문) · 구글 로고 + [발음 검색]·[시각 자료 검색]) → AI 채팅 영역(질문/답변 말풍선 + "궁금한 내용을 입력하세요…" 입력창) → 하단 "자주 쓰는 질문" 목록(각 항목 [수정], 드래그로 순서 변경; 예: 문법적 역할 / 문맥 속 의미 / 격식·객관 표현 여부). 창 크기는 설정 화면과 동일, 라이트 테마.
...
- 외부 브라우저로 열 때 이미 떠 있는 브라우저의 새 탭이 아니라 새 창으로 뜨도록 한다(macOS: `open -n`).
