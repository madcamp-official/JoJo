...
**문제의식**: 외국어 원서·영상·웹소설을 소비할 때, 모르는 표현을 만나면 뷰어를 벗어나 사전/번역기로 이동해야 하고, 사전은 그 **문맥 속 의미**를 짚어주지 못한다. `read`의 발음, `後`의 독음, 어떤 뜻으로 쓰였는지는 문맥이 있어야 안다.
...
|---|---|---|---|---|---|---|---|
| **tier1** | 영어·일본어·중국어(간체/번체), 4개 | ✅ 특화 OCR | ✅ | ✅ | ✅(ja/zh만) | ✅ | ✅ |
| **tier2-A** | eld 감지 가능 + 네이버 사전 실지원, 30개 | ✅ 범용 OCR | ✅(IPA) | ❌ | ❌ | ✅ | ✅ |
| **tier2-B** | eld 감지 가능, 네이버 사전 없음, 25개 | ✅ 범용 OCR | ✅(IPA) | ❌ | ❌ | ✅ | ❌ |
...
- **tier1**은 기존 4개 언어 그대로(`Language` 타입, 닫힌 유니온) — AI 사전(§6 사전 API 구성,
  ...
  분석기 부재로 아예 글자 단위 강제(아래 항목)로 대체한다. 설정 화면 언어 선택 드롭다운은
  이 차이를 tier1 = **특화 OCR**(언어별 형태소 분석기 재분할까지 포함) vs tier2 = **범용
  OCR**(공백/글자 단위 폴백)로 구분해 표기한다(`SettingsScreen.tsx`).
- **tier2**(A/B 합쳐 55개, `LinkLanguage` 타입)는 OCR·AI 발음·구글 검색까지는 tier1과 동일하게
...
- **tier2-B(25개, 구글만)**: 암하라어, 아제르바이잔어, 벨라루스어, 불가리아어, 벵골어,
  카탈루냐어, 에스토니아어, 바스크어, 구자라트어, 아르메니아어, 아이슬란드어, 칸나다어,
  한국어, 리투아니아어, 라트비아어, 말라얄람어, 마라티어, 말레이어, 오리야어,
...
- **tier3**: 위에 나열되지 않은 그 외 모든 언어(eld가 감지하더라도 tier2에 없으면 tier3).
  ...
  기준으로 갱신할 것. **쿠르드어(ku)는 2026-07-30 tier2 도입 때 등록했다가 전수조사
  (2026-07-30) 후 tier3로 되돌렸다** — eld의 ku 모델(소라니/아랍 문자)과 이 앱이 쓰는 Tesseract
  팩(쿠르만지/라틴 문자)이 서로 다른 방언·문자체계를 가리켜, OCR 경로는 구조적으로 자동판별이
  불가능하고 자막/DOM 텍스트 경로(eld 직접 호출)는 원리상 감지가 되긴 하지만 두 경로 결과가
  갈리는 비일관성을 피하려 LINK_LANGUAGES 자체에서 제거해 tier3로 통일했다. 근거는
  `src/shared/languages.ts` LINK_LANGUAGES 주석·`src/main/selection/langDetect.ts`의
  `SCRIPT_TO_LANGUAGE.Latin` 주석 참고.
...
3. **단축키 설정** — 모드 전환/창 선택 전환/창 선택 해제/영역 수동 선택/OCR로 전환/설정 화면 열기, 총 6개 키 지정(연필/휴지통 아이콘 버튼). 모드 전환 기본 예: `` Alt+` ``. 표시는 OS에 맞춰 macOS는 `Cmd/Ctrl`·`Opt`, 그 외는 `Ctrl`·`Alt`로 보여주며, macOS에서는 실제로 Cmd/Ctrl 두 키 모두로 동작한다. 녹화 시 (1) 다른 단축키가 이미 쓰는 조합, (2) Shift만 걸고 문자 키를 조합해 그 문자 타이핑 자체를 가로채게 되는 조합(예: Shift+T), (3) `Cmd+Q`/`Cmd+Space`/`Alt+F4`/`Ctrl+C` 등 OS·다른 앱의 필수 단축키는 등록을 막고 이유를 안내한다(`SettingsScreen.tsx`/`shortcutMatch.ts`).
...
**폴백 판단 기준은 "사전에 표제어가 있었는지"가 아니라 "문맥에 맞는 뜻을 실제로 골랐는지"다**(2026-07-31) — 아래 언어별 순서를 앞에서부터 시도하되, 소스마다 조회와 AI 뜻 번호 판정까지 끝내고, 사전엔 뜻이 있었지만 AI가 "이 문맥에 맞는 뜻은 없다"고 답하면 그것도 실패로 보고 다음 소스로 넘어간다(체인이 끝까지 실패하면 우선순위가 가장 높았던 소스의 안내문을 보여준다). 이 과정은 채팅창에 실시간 진행 로그로 표시된다 — 어느 사전에서 검색 중인지, 결과가 없어 폴백했는지, 문맥에 맞는 뜻이 없어 폴백했는지가 한 줄씩 쌓이고 최종 답변이 나오면 사라진다(`main/question/dictionary.ts` `ProgressFn` → `QuestionResult.meta.progress`).
...
- 브라우저 확장(유튜브/넷플릭스 자막, 단어 하이라이트) + 앱과 로컬 WebSocket — 실제 구현은 자막 문맥 로직과의 결합도 때문에 담당 B로 이관됨(TODO.md 참고), DOM 텍스트(일반 웹페이지)는 A 영역으로 담당: milleion이 2026-07-30 구현 완료(설계 방향·상세는 TODO.md "DOM 텍스트(일반 웹페이지) 직접 추출" 항목 참고).
...
- **확장**: 지원 언어 추가, 학습 이력·단어장, 발음 TTS, 모바일, 스캔 만화(칸 분할 인식 등 텍스트 소설과 다른 처리가 필요해 이번 범위에서는 제외).
...
│   │   ├── windows.ts           #   메인(라우트 전환, showMainWindowAtRoute — 숨은 창은 렌더러의 NAVIGATE_READY 응답 받은 뒤에만 show()해 전환 깜빡임 방지)/오버레이/팝업 윈도우 팩토리
│   │   ├── ipc.ts               #   🤝 IPC 허브(A→B 연결점)
│   │   ├── contextMenu.ts       #   🤝 모든 창 공통 OS 우클릭 메뉴
│   │   ├── tray.ts              #   🅰️ 트레이 아이콘(모드 전환/창 선택 전환/창 선택 해제/영역 수동 선택/OCR로 전환/설정)
...
│   │   │   ├── shortcut.ts      #   모드 전환 전역 단축키(Alt+`, macOS Cmd/Ctrl 이중 등록) + 창 선택/해제·영역 수동 선택·OCR 강제전환 등 named shortcut 레지스트리(registerNamedShortcut/updateNamedShortcut)
