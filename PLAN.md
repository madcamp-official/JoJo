...
**팝업 화면**: 상단에 선택된 원문 문맥 표시(선택 앞뒤 각 512바이트, 문장 경계까지 확장·부족하면 있는 만큼만, 내부 스크롤 없이 텍스트 양만큼 높이 자동 확장) → 툴바(LLM 아이콘 · [발음]·[사전 검색] 체크박스 · [입력](통합 질문) · 구글 로고 + [발음 검색]·[시각 자료 검색]) → AI 채팅 영역(질문/답변 말풍선 + "궁금한 내용을 입력하세요…" 입력창) → 하단 "자주 쓰는 질문" 목록(각 항목 [수정], 드래그로 순서 변경; 예: 문법적 역할 / 문맥 속 의미 / 격식·객관 표현 여부). 창 크기는 설정 화면과 동일, 라이트 테마.
