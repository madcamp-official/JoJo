...
빌드 전략: **뼈대 → 한 경로 end-to-end 관통 → 소스별 확장**. 첫 관통 경로는 가장 확실한 **PDF/텍스트 직접 추출 → 통합 질문**으로 잡고, 이후 OCR·자막·발음·사전을 붙인다.

| 일차 | A (선택/추출) | B (검색/AI) | 공동 |
|------|----------------|--------------|------|
| D1 | Electron 뼈대·창 선택·캡처 PoC | LLM 어댑터(1종) + 채팅 팝업 PoC | **인터페이스 계약(§8) 확정**, 레포·CI |
| D2 | 오버레이 윈도우·단축키·모드 전환 | 통합 질문 + 컨텍스트 전달·캐싱 | 첫 관통(PDF→통합질문) 연결 |
| D3 | txt/PDF 직접 추출 + 좌표 매핑 | 발음 기능(맥락 발음) | 단어 하이라이트/커서 피드백 |
| D4 | OCR 파이프라인 + 언어 감지 | 사전 API + 뜻 번호 판정 | OCR 노이즈 제거 로직 |
| D5 | 브라우저 확장(DOM/유튜브 자막) | 구글 검색 탭 + 커스텀 질문 관리 | 확장↔앱 native messaging |
| D6 | 넷플릭스 자막·탭/URL 재판정 | 설정 화면(언어·LLM·키·단축키·범위) | 통합·버그픽스 |
| D7 | 크로스플랫폼(Win/Mac) 점검 | 발표 시나리오·데모 데이터 | 리허설·문서 |
