...
- OCR로 뽑은 일본어·중국어는 Tesseract 자체 단어 경계 대신, 줄 단위로 다시 형태소 분석기(일: kuromoji, 중: segmentit)를 돌려 의미 단위 단어로 재구성한다(`main/nlp/`). 각 단어 bbox는 구성 글자(symbol)들의 bbox를 합쳐(union) 계산.
