...
3. [사용 흐름](#3-사용-흐름)
4. [핵심 기능](#4-핵심-기능)
5. [사전 API 구성](#5-사전-api-구성)
6. [기술 아키텍처](#6-기술-아키텍처)
7. [까다로운 부분 & 해결 전략](#7-까다로운-부분--해결-전략)
...
| Tier | 언어 | OCR | AI 발음 | LLM 사전(AI 사전) | 구글(발음/이미지 검색) | 네이버 사전 |
|---|---|---|---|---|---|---|
| **tier1** | 영어·일본어·중국어(간체/번체), 4개 | ✅ | ✅ | ✅ | ✅ | ✅ |
| **tier2-A** | eld 감지 가능 + 네이버 사전 실지원, 30개 | ✅ | ✅(IPA) | ❌ | ✅ | ✅ |
| **tier2-B** | eld 감지 가능, 네이버 사전 없음, 26개 | ✅ | ✅(IPA) | ❌ | ✅ | ❌ |
| **tier3** | 그 외 전부(eld가 아예 못 잡는 언어 포함) | ❌ | ❌ | ❌ | ❌ | ❌ |

- **tier1**은 기존 4개 언어 그대로(`Language` 타입, 닫힌 유니온) — LLM 사전(§5 사전 API 구성,
  사전 API + LLM 뜻 번호 판정)까지 포함한 전체 기능을 받는 유일한 등급.
- **tier2**(A/B 합쳐 56개, `LinkLanguage` 타입)는 OCR·AI 발음·구글 검색까지는 tier1과 동일하게
  받지만, LLM 사전(사전 API 폴백 체인)은 tier1 전용으로 남겨뒀다 — 사전 API 자체가 tier1 3개
  언어(+한국어 미지원) 기준으로 구성돼 있어서(§5), tier2까지 확장하려면 언어별 사전 소스를 새로
  발굴해야 해 이번 범위에서 제외. "AI 발음"은 사전 API 없이 LLM 프롬프트만으로 되는 기능이라
  tier 구분 없이 항상 제공된다.
...
- **타입 구조**: `Language`(tier1, 4개) ⊂ `AnyLanguage = Language | LinkLanguage`(tier1+tier2,
  ...
  로직(LLM 사전)은 진입점에서 `isFullLanguage()`로 좁혀 `Language`만 다루게 해서, 사전 없는
...
선택 영역을 사전 API로 조회해 원어 뜻(sense) 목록을 확보하고, 문맥상 몇 번째 뜻인지는 LLM이 판정한다(§4.2 "사전 검색"). 사전 API는 원어 sense 목록만 제공하고 한국어 설명·번역은 LLM이 담당한다 — 공식 무료 이중언어(외국어→한국어) API가 마땅치 않기 때문이다. 아래 구성은 **비상업(개인 목적) 사용을 전제**로 확정한 것이며, 상업화 시 재검토가 필요한 항목은 마지막에 별도 표기한다.
